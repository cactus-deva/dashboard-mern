...
const getAllProperties = async (req, res) => {
  ...
  try {
    ...
    res.header("Access-Control-Export-Headers", "x-total-count");
...
const createProperty = async (req, res) => {
  try {
    const { title, propertyType, description, location, price, photo, email } =
      req.body;
    ...
    res.status(200).json({ message: "Property crated successfully" });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
...
const deleteProperty = async (req, res) => {
  try {
    ...
    propertyExits.creator.allProperties.pull(propertyExits._id)
